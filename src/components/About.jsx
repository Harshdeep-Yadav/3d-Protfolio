...
const ServiceCard = ({ ind, title, icon }) => {
  return (
    <Tilt className="xs:w-[250px] w-full">
      <motion.div
        variants={fadeIn("right", "spring", 0.5 * ind, 0.75)}
        
        className="w-full green-pink-gradient p-[1px] rounded-[20px] shadow-card"
      >
        <div
          onCompositionStart={{ max: 45, scale: 1, speed: 450 }}
  ...
  return (
    <>
      <motion.div variants={textVariant()}>
        <p className={styles.sectionSubText}>Introductoin</p>
      ...
      <div className="flex flex-wrap gap-10 mt-28">
        {services.map((service, ind) => (
          <ServiceCard key={ind} ind={ind} {...service} />
        ))}
      </div>
    </>
  );
